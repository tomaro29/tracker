...
public class AccountLabelProvider extends AbstractLabelProvider {
	...
	if (element instanceof Account)
	    return getAccountName((Account) element) + STRING_SEPARATOR + getAccountIdentifier((Account) element);
	return super.getText(element);
    }

    private String getAccountName(Account account) {
	return StringUtils.isEmpty(account.getName()) ? STRING_UNDEFINED_NAME : account.getName();
    }

    private String getAccountIdentifier(Account account) {
	return account.getIdentifier() == 0 ? STRING_UNDEFINED_IDENTIFIER : String.valueOf(account.getIdentifier());
    }
}
