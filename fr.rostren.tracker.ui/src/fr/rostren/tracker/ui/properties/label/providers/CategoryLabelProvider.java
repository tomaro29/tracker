...
public class CategoryLabelProvider extends AbstractLabelProvider {
	...
	public String getText(Object element) {
		if (element instanceof Category) {
			return getCategoryTitle((Category) element) + AbstractLabelProvider.STRING_SEPARATOR
					+ getCategoryDescription((Category) element);
		}
		return super.getText(element);
	}

	private String getCategoryTitle(Category category) {
		return StringUtils.isEmpty(category.getTitle()) ? CategoryLabelProvider.STRING_UNDEFINED_TITLE
				: category.getTitle();
	}

	private String getCategoryDescription(Category category) {
		return StringUtils.isEmpty(category.getDescription()) ? CategoryLabelProvider.STRING_UNDEFINED_DESCRIPTION
