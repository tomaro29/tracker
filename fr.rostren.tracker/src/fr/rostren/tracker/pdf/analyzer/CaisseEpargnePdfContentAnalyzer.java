...
public class CaisseEpargnePdfContentAnalyzer extends AbstractPdfContentAnalyzer {
    ...
    private final Pattern DATE_LINE_PATTREN = Pattern.compile(
	    "au (0[1-9]|1[0-9]|2[0-9]|3[0-1])/(0[1-9]|1[0-2])/[0-9]{4} - N° [1-9]{2} Page [1-9] / [1-9][\\s\\n\\r]*"); //$NON-NLS-1$
    ...
    private final Pattern COMPLETE_LINE_PATTERN = Pattern
	    .compile(PART_1_DATE_PATTREN.pattern().concat(DATE_SEPARATOR_PATTREN.pattern())
		    .concat(PART_2_DATE_PATTREN.pattern()).concat(EMPTY_STRING_PATTREN.pattern())
		    .concat(OPERATION_TITLE_PATTREN.pattern()).concat(EMPTY_STRING_PATTREN.pattern())
		    .concat(AMOUNT_NUMBER_PATTREN.pattern()).concat(EMPTY_STRING_PATTREN.pattern()));
	...
	if (dateMatcher.matches()) {
	    ...
	    // DONOTHING
	} else if (getLastToken() != null && (PdfToken.OPERATIONS_DEPOT.equals(getLastToken())
		|| PdfToken.VIR_RECU.equals(getLastToken()) || PdfToken.PAIE_CHEQUE.equals(getLastToken())
		|| PdfToken.FRAIS_BANCAIRES.equals(getLastToken()) || PdfToken.PAIEMENTS_CARTES.equals(getLastToken())
		|| PdfToken.RETRAITS_CARTES.equals(getLastToken()) || PdfToken.PRELEVEMENTS.equals(getLastToken())
		|| PdfToken.OPERATIONS_DIVERSES.equals(getLastToken()))) {
	...
	if (getCurrentLine().matches(COMPLETE_LINE_PATTERN.pattern())) {
	    ...
	    if (!beforeLastPart.matches(FACT_PATTREN.pattern()) && beforeLastPart.length() <= 3
