...
public class OwnerAttributesModifyListener extends AbstractModifyListener {
	...
	protected void executeModifyText(Widget widget) {
		...
		if (widget.equals(firstName))
			executeSetCommand(eObject, TrackerPackage.Literals.OWNER__FIRST_NAME, firstName.getText());
		else if (widget.equals(lastName))
			executeSetCommand(eObject, TrackerPackage.Literals.OWNER__LAST_NAME, lastName.getText());
	}
}
