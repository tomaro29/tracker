...
public class OwnerLabelProvider extends AbstractLabelProvider {
	...
	public String getText(Object element) {
		if (element instanceof Owner) {
			return getOwnerFirstName((Owner) element) + AbstractLabelProvider.STRING_SEPARATOR
					+ getOwnerLastName((Owner) element);
		}
		return super.getText(element);
	}

	private String getOwnerFirstName(Owner owner) {
		return StringUtils.isEmpty(owner.getFirstName()) ? OwnerLabelProvider.STRING_UNDEFINED_FIRST_NAME
				: owner.getFirstName();
	}

	private String getOwnerLastName(Owner owner) {
		return StringUtils.isEmpty(owner.getLastName()) ? OwnerLabelProvider.STRING_UNDEFINED_LAST_NAME
				: owner.getLastName();
	}
}
