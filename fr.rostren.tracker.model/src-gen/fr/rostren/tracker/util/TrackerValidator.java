	...
	public boolean validateTitle_isUnique(Title title, DiagnosticChain diagnostics, Map<Object, Object> context) {
		...
		if (title instanceof Category) {
			...
			for (IncomeCategory sibling: repository.getIncome().getIncomes()) {
				if (title instanceof IncomeCategory && !sibling.equals(title) && sibling.getTitle().equals(title.getTitle())) {
					isNotUnique=true;
					break;
				}
			}
			for (SpendingCategory sibling: repository.getSpending().getSpendings()) {
				if (title instanceof SpendingCategory && !sibling.equals(title) && sibling.getTitle().equals(title.getTitle())) {
					isNotUnique=true;
					break;
				}
			}
